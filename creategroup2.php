...
<?php
include("db.php");
$qry=" select ifnull(max(groupid),0) from tbgroup ";
$result=mysqli_query($con,$qry) or die(mysqli_error($con));
$row=mysqli_fetch_row($result);
$n=$row[0];
$n++;
$old=$_FILES["File1"]["tmp_name"];
$new="images/".$n."_".$_FILES["File1"]["name"];
move_uploaded_file($old,$new);

$title=$_POST["Text1"];
$desc=$_POST["TextArea1"];
$qry="insert into tbgroup values($n,'$title','$uid','$desc','$new')";
mysqli_query($con,$qry) or die(mysqli_error($con));
$qry=" select ifnull(max(memberid),0) from tbgroupmember ";
$result=mysqli_query($con,$qry) or die(mysqli_error($con));
$row=mysqli_fetch_row($result);
$p=$row[0];
$p++;

$qry="insert into tbgroupmember values($p,'$uid',$n) ";
mysqli_query($con,$qry) or die(mysqli_error($con));
echo "group has been created succefully!!";
?>
