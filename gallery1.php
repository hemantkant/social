...
<?php
include("db.php");
$qry=" select ifnull(max(albumid),0) from tbalbum ";
$result=mysqli_query($con,$qry) or die(mysqli_error($con));
$row=mysqli_fetch_row($result);
$n=$row[0];
$n++;
$old=$_FILES["File1"]["tmp_name"];
$new="image/".$n."_".$_FILES["File1"]["name"];
move_uploaded_file($old,$new);

$title=$_POST["Text1"];
$desc=$_POST["TextArea1"];
$qry="insert into tbalbum values($n,'$title','$desc','$uid','$new')";
mysqli_query($con,$qry) or die(mysqli_error($con));
$qry=" select ifnull(max(albumcid),0) from tbalbumcontent ";
$result=mysqli_query($con,$qry) or die(mysqli_error($con));
$row=mysqli_fetch_row($result);
$p=$row[0];
$p++;

$qry="insert into tbalbumcontent values($p,'$uid','$new',$n) ";
mysqli_query($con,$qry) or die(mysqli_error($con));
echo "album has been created succefully!!";
?>
